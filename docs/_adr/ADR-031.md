...
> Historical note (2026-05-09): The channel event-shape examples in this ADR (`Full`/`Empty`) were superseded by
> [ADR-037](/adr/adr-037). Current `slug.io.stdin.readLines()` emits `@str` values and channel closure is observed as
> `nil`.
