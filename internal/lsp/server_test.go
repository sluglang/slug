...
func TestServerSignatureHelpShowsColonTypedParameters(t *testing.T) {
	src := "val sum = fn(a:num, b:str) { a }\nsum(1, \"x\")\n"
	srcJSON := strings.ReplaceAll(src, "\"", "\\\"")
	srcJSON = strings.ReplaceAll(srcJSON, "\n", "\\n")
	in := strings.NewReader(
		frame(`{"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}`) +
			frame(`{"jsonrpc":"2.0","method":"textDocument/didOpen","params":{"textDocument":{"uri":"file:///a.slug","version":1,"text":"`+srcJSON+`"}}}`) +
			frame(`{"jsonrpc":"2.0","id":135,"method":"textDocument/signatureHelp","params":{"textDocument":{"uri":"file:///a.slug"},"position":{"line":1,"character":7}}}`) +
			frame(`{"jsonrpc":"2.0","id":2,"method":"shutdown","params":{}}`) +
			frame(`{"jsonrpc":"2.0","method":"exit"}`),
	)
	var out bytes.Buffer
